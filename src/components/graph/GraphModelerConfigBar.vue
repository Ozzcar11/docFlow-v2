<template>
   ...
   <div v-else-if="cell.isNode()"
      class="w-full p-3">

      <ModalComponent :modal="modal_field_content"
         v-if="modal_field_content.is_open"
         :container_style="{ 'max-width': '48rem' }">
         <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="mt-3 text-center sm:mt-0 sm:text-left">
               <h3 class="text-lg leading-6 font-medium text-gray-900"
                  id="modal-title">Edit content</h3>
               <div class="mt-2">
                  <p class="text-sm text-gray-500">
                     You can use <a href="https://commonmark.org/help/"
                        target="_blank">markdown</a> to add styles to the content displayed for this node.
                  ...
                  <template v-if="modal_field_content.data.read_more">
                     <p class="text-sm text-gray-500 mt-1">
                        Docassmble uses Mako as a templating system, to allow for including variables and code in content.
                        More on how to use this at <a href="https://docassemble.org/docs/markup.html#mako"
                           target="_blank">Docassemble</a>.
                     </p>
                     <p class="text-sm text-gray-500 mt-1">
                        Note that when referencing a variable, even in an 'if' statement, the block corresponding to the
                        variable will be first shown to the user (regardless of the order specified in the graph).
                        To show a variable only when it is defined, one can use the <span
                           class="code text-gray-700">defined('variable')</span> function within Mako, as described at <a
               ...
               <div class="mt-4">
                  <EasymdeView :options="{}"
                     ...
                     @input="e => modal_field_content.data.editor = e"></EasymdeView>
               </div>
            </div>
         </div>
         <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <button @click="modal_field_content_accept(modal_field_content)"
               type="button"
               class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm">Accept</button>
            <button @click="modal_field_content.close()"
               type="button"
      ...
      <div class="">
         ...
         <span class="mb-2 text-ellipsis overflow-hidden font-mono block text-xl border-b border-gray-300">{{
            ...
            `${typeToRu((cell.getData() as AntvNodeData).gd.type)}_${cell.id.toString().substring(0, 8)}`
         }}</span>

         <div class="w-full">
            <button @click="cell?.remove()"
               class="btn btn-red">
         ...
         <div v-for="(fields, field_group) in current_fields"
            ...
            <div class="my-2">
               ...
               <span class="mt-1 mb-3 block border-b border-gray-300 text-lg leading-6 font-medium text-gray-800">{{
                  'Главное' }}</span>
                  <!-- field_group.toString()[0].toUpperCase().concat(field_group.toString().substring(1)) -->
               ...
               <div v-for="field in fields"
                  ...
                  <label v-if="field == 'variable'">
                     <span class="text-gray-700 block mb-1 text-sm">Название</span>
                     <div>
                        <!-- <input class="w-full" type="text" :placeholder="cell.id" v-model="node_variable" /> -->
                        <input class="style-soft w-full font-mono"
                           type="text"
                           v-model="node_variable"
                           :placeholder="`${typeToRu((cell.getData() as AntvNodeData).gd.type)}_${cell.id.toString().substring(0, 8)}`" />
                     </div>
                  </label>

                  <div v-if="field == 'label'">
                     <div class="flex justify-between items-center mb-1">
                        <label class="text-gray-700 block text-sm"
                           for="textarea_content">Контент</label>
                        <button class="btn text-sm"
                           @click="modal_field_content.open()">
                           <svg xmlns="http://www.w3.org/2000/svg"
                              class="h-5 w-5"
                              fill="none"
                              viewBox="0 0 24 24"
   ...
   <div v-else-if="cell.isEdge()"
      class="w-full p-2">
      <div class="">
         <span class="mb-2 font-bold block text-2xl border-b border-gray-300">Линия</span>
         ...
         <div class="my-2"
            v-if="(cell.getSourceCell()?.getData() as AntvNodeData).gd.type == 'decision'">
            <span class="my-2 block border-b border-gray-300 uppercase font-bold text-sm text-gray-800">Основные</span>
            ...
            <div class="block mt-2">
               ...
               <div class="w-full my-2 text-right">
                  <button class="inline btn mr-2"
                     @click="edge_content = 'Yes'">Да</button>
                  <button class="inline btn"
                     @click="edge_content = 'No'">Нет</button>
               </div>
            </div>
...
import { typeToRu } from '@/utils/typeToRu';

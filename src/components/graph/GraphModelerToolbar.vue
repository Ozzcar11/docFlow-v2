...
const modal_import: Modal<{ text: string }> = new Modal(
  { text: "" },
  {
    onOpen: () => {
      nextTick(() => {
...
const modal_import_file_changed = (event: Event) => {
  ...
  reader.onload = (ev) => {
    if ((ev.target?.result ?? false) && typeof ev.target?.result === "string") modal_import.data.text = ev.target?.result
  }
...
const modal_import_import = () => {
  ...
  try {
    ...
    if (!import_json) throw Error()
  } catch (e) {
    toast.error("Data does not contain valid JSON, or does not contain the GraphDoc boundary specifier.")
    return
  }

  if (!Object.hasOwnProperty.call(import_json, "main") || !Object.hasOwnProperty.call(import_json, "nodes") || !Object.hasOwnProperty.call(import_json, "edges")) {
...
const item_clear = () => {
  // if (typeof props.initModeler !== "undefined" && confirm("Are you sure you want to reset your workspace?")){
  //     props.initModeler();
  // }

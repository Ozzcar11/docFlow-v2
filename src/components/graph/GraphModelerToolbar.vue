<template>
   <ModalComponent :modal="modal_import"
      v-if="modal_import.is_open"
         ...
         <div class="sm:flex sm:items-start">
            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
               <h3 class="text-lg leading-6 font-medium text-gray-900"
                  id="modal-title">Import</h3>
               <p class="mt-2 text-sm text-gray-500">Paste the data of the exported interview below.</p>
               <textarea placeholder="Paste the contents of the interview here"
               ...
               <span class="block mt-2 text-sm text-gray-500">or <label
                     class="text-sm cursor-pointer text-gray-800 bg-gray-100 hover:bg-gray-200 border border-gray-400 py-0.5 px-1 mx-0.5 rounded-md">
                     <span>upload a file</span>
                     <input type="file"
                        class="hidden"
                        @change="modal_import_file_changed" />
                  </label> instead.</span>
            </div>
         </div>
      </div>
      <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
         <button @click="modal_import_import()"
            type="button"
            class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm">
            Import</button>
         <button @click="modal_import.close()"
            type="button"
